...
class VisdomReporter:
    ...
    def plot_image(self, dirty_tensor, dirty_like, clean_tensor, clean_like):
        ...
        dirty_group = vutils.make_grid(dirty_tensor, nrow = 8, padding=2, normalize=True).cpu()
        dirty_like_group = vutils.make_grid(dirty_like, nrow = 8, padding=2, normalize=True).cpu()
        clean_group = vutils.make_grid(clean_tensor, nrow = 8, padding=2, normalize=True).cpu()
        clean_like_group = vutils.make_grid(clean_like, nrow = 8, padding=2, normalize=True).cpu()
