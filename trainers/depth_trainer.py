...
class DepthTrainer:
    ...
    def __init__(self, gan_version, gan_iteration, gpu_device, g_lr = 0.0002, d_lr = 0.0002):
        ...
        self.G_B = cg.Generator(input_nc = 1, output_nc = 3, n_residual_blocks = 6).to(self.gpu_device)
    ...
    def train(self, rgb_tensor, depth_tensor):
        ...
        errG = A_likeness_loss + B_likeness_loss + A_adv_loss + B_adv_loss
        errG.backward()
    ...
    def visdom_report(self, iteration, train_rgb_tensor, train_depth_tensor, test_rgb_tensor, test_depth_tensor):
        with torch.no_grad():
            train_depth_like = self.G_A(train_rgb_tensor)
            train_rgb_like = self.G_B(train_depth_tensor)
            test_depth_like = self.G_A(test_rgb_tensor)
            test_rgb_like = self.G_B(test_depth_tensor)
            ...
            self.visdom_reporter.plot_image((train_rgb_tensor), "Training RGB images")
            self.visdom_reporter.plot_image((train_rgb_like), "Training RGB-like images")
            ...
            self.visdom_reporter.plot_image((test_rgb_tensor), "Test RGB images")
            self.visdom_reporter.plot_image((test_rgb_like), "Test RGB-like images")
            self.visdom_reporter.plot_image((test_depth_tensor), "Test Depth images")
            self.visdom_reporter.plot_image((test_depth_like), "Test Depth-like images")
