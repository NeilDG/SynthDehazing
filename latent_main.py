...
from trainers import latent_trainer
import constants
...
def main(argv):
    ...
    trainer = latent_trainer.LatentTrainer(constants.DEHAZER_VERSION, constants.ITERATION, device, opts.g_lr, opts.d_lr)
    ...
    if(opts.load_previous):
        ...
        trainer.load_saved_state(iteration, latent_checkpoint, constants.GENERATOR_KEY, constants.LATENT_VECTOR_KEY, constants.OPTIMIZER_KEY)

        print("Loaded checkpt: %s Current epoch: %d" % (constants.LATENT_CHECKPATH, start_epoch))
        print("===================================================")

    # Create the dataloader
    train_loader = dataset_loader.load_latent_dataset(constants.DATASET_VEMON_PATH_PATCH, constants.batch_size, opts.img_to_load)
