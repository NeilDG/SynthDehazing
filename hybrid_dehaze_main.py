...
def main(argv):
    ...
    colorizer = correction_trainer.CorrectionTrainer(constants.COLORIZER_VERSION, constants.ITERATION, device,
                                                     opts.g_lr, opts.d_lr)
    ...
    if (opts.load_previous):
        ...
        dehazer.load_saved_state(iteration, dehaze_checkpoint, constants.GENERATOR_KEY, constants.DISCRIMINATOR_KEY,
                                 constants.OPTIMIZER_KEY)
        colorizer.load_saved_state(iteration, color_checkpoint, constants.GENERATOR_KEY, constants.DISCRIMINATOR_KEY,
                                   constants.OPTIMIZER_KEY)

        print("Loaded checkpt: %s %s Current epoch: %d" % (
            constants.DEHAZER_CHECKPATH, constants.COLORIZER_CHECKPATH, start_epoch))
        print("===================================================")

    # Create the dataloader
    synth_train_loader = dataset_loader.load_dark_channel_dataset(constants.DATASET_HAZY_PATH_PATCH,
                                                                  constants.DATASET_CLEAN_PATH_PATCH,
                                                                  constants.batch_size, opts.img_to_load)
    synth_test_loader = dataset_loader.load_dark_channel_test_dataset(constants.DATASET_HAZY_PATH_COMPLETE,
                                                                      constants.DATASET_CLEAN_PATH_COMPLETE, 4, 500)
    rgb_train_loader = dataset_loader.load_rgb_dataset(constants.DATASET_OHAZE_PATH_PATCH_CLEAN, constants.batch_size,
                                                       opts.img_to_load)
    ...
    if (constants.is_coare == 0):
        for epoch in range(start_epoch, constants.num_epochs):
            ...
            for i, (dehaze_data, colored_data) in enumerate(zip(synth_train_loader, rgb_train_loader)):
                ...
                if (i % 500 == 0):
                    ...
                    if (index == 0):
                        rgb_test_loader = dataset_loader.load_color_test_dataset(constants.DATASET_VEMON_PATH_COMPLETE,
                                                                                 constants.display_size, 500)
                        synth_test_loader = dataset_loader.load_dark_channel_test_dataset(
                            constants.DATASET_HAZY_PATH_COMPLETE, constants.DATASET_CLEAN_PATH_COMPLETE,
                            constants.batch_size, 500)
                if (i % 1500 == 0):
                    dehazer.save_states(epoch, iteration, constants.DEHAZER_CHECKPATH, constants.GENERATOR_KEY,
                                        constants.DISCRIMINATOR_KEY, constants.OPTIMIZER_KEY)
                    colorizer.save_states(epoch, iteration, constants.COLORIZER_CHECKPATH, constants.GENERATOR_KEY,
                                          constants.DISCRIMINATOR_KEY, constants.OPTIMIZER_KEY)
    else:
        for i, (dehaze_data, colored_data) in enumerate(zip(synth_train_loader, rgb_train_loader)):
            _, hazy_batch, clean_batch = dehaze_data
        ...
        # save every X epoch
        dehazer.save_states(start_epoch, iteration, constants.DEHAZER_CHECKPATH, constants.GENERATOR_KEY,
                            constants.DISCRIMINATOR_KEY, constants.OPTIMIZER_KEY)
        colorizer.save_states(start_epoch, iteration, constants.COLORIZER_CHECKPATH, constants.GENERATOR_KEY,
                              constants.DISCRIMINATOR_KEY, constants.OPTIMIZER_KEY)
