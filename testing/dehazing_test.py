...
def perform_airlight_predictions(airlight_checkpt_name, albedo_checkpt_name):
    ...
    with open(PATH_TO_FILE, "w") as f, torch.no_grad():
        for i, (test_data) in enumerate(test_loader):
            ...
            mean_error = mse(mean_batch, airlight_batch).item()
        ...
        average_MSE[0] = np.round(average_MSE[0] / i * 1.0, 5)
        average_MSE[1] = np.round(average_MSE[1] / i * 1.0, 5)
        average_MSE[2] = np.round(average_MSE[2] / i * 1.0, 5)
...
def main():
    ...
    perform_airlight_predictions("airlight_estimator_v1.02_1", "albedo_transfer_v1.01_1")
