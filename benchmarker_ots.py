...
def benchmark_ots():
    ...
    BENCHMARK_PATH = "results/metrics_ots - " + str(MODEL_CHECKPOINT) + ".txt"

    device = torch.device("cuda:0" if (torch.cuda.is_available()) else "cpu")

    hazy_list = glob.glob(HAZY_PATH + "*1_0.2.jpg") #specify atmosphere intensity
    gt_list = glob.glob(GT_PATH + "*.jpg")

    print("Found images: ", len(hazy_list), len(gt_list))
